      ...
      <header className="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl">
        <div className="flex flex-col justify-between gap-5 md:flex-row md:items-center">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-black">Treinos</h1>
            <p className="mt-3 text-slate-300">
              Aqui os professores vão controlar treinos, revisões e entregas dos alunos.
            </p>
          </div>
      ...
      <section className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl">
        <div className="mb-6 flex flex-col justify-between gap-3 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Aqui ficarão os treinos cadastrados, entregues e pendentes.
            </p>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-white/10">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {treinos.map((treino) => (
                <tr key={treino.aluno} className="border-t border-white/10">
                  <td className="px-4 py-4 font-semibold">{treino.aluno}</td>
                  <td className="px-4 py-4 text-slate-300">{treino.objetivo}</td>
                  <td className="px-4 py-4 text-slate-300">{treino.professor}</td>
                  <td className="px-4 py-4">
                    <span className="rounded-full bg-cyan-400/15 px-3 py-1 text-xs text-cyan-100">
                      {treino.status}
                    </span>
                  </td>
                  <td className="px-4 py-4 text-slate-300">{treino.revisao}</td>
                </tr>
              ))}
            </tbody>
          </table>
